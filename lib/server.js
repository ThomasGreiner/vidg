...
class Server {
  ...
  _getFile(dir) {
    ...
    } else if (this._currentIndex >= this._fileIndex.length) {
  ...
  _getStats(file) {
    ...
    ctx.strokeStyle = "#888";
    ctx.beginPath();
    ctx.moveTo(currentX - statsHeight / 4, statsHeight);
    ctx.lineTo(currentX, statsHeight / 2);
    ctx.lineTo(currentX + statsHeight / 4, statsHeight);
    ctx.fill();
    ctx.stroke();

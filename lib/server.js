...
class Server {
  ...
  listen(port) {
    return this._db.fileIndex
        .then((fileIndex) => this._index = new Cursor(fileIndex))
        .then(() => this._db.comparisons)
        .then((comparisons) => this._comparisons = new Cursor(comparisons))
        .then(() => this._optimizations = new Cursor([]))
        .then(() => {
    ...
    return Promise.all([a, b])
        .then((files) => {
          ...
          let data = {
            ...
            stats: this._getStats(this._comparisons, ids)
          };
          return {
  ...
  _getFile(fileEntry) {
    return this._db.get(fileEntry.id)
        .then((file) => {
          let data = {
            path: path.relative(this._db.PATH_ORIGIN, file.path),
            preview: file.preview,
            rating: file.rating,
            size: file.stats.size,
            stats: this._getStats(this._index, new Set([file.id]))
          };
          return {
            content: JSON.stringify(data),
            status: 200,

...
function getStats(file) {
  return {
    ...
    keep: SYNC.files.filter((file) => file.status == "keep").length,
    progress: SYNC.files.indexOf(file) + 1,
    total: SYNC.files.length,
    trash: SYNC.files.filter((file) => file.status == "trash").length
  };
}
...
function writeNextFile(resp, dir) {
  ...
  } else if (currentIndex >= SYNC.files.lengt) {
  ...
  switch (method) {
    ...
    case "POST":
      ...
      req.on("end", () => {
        ...
        SYNC.update(file, "status", status)
            .then(() => {
              resp.writeHead(200);
              resp.write(JSON.stringify({
                image: file.screenshot,

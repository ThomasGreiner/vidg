...
const colors = ["#D88", "#FFF", "#EFE", "#CDC", "#ADA", "#8D8", "#6D6"];
  ...
  _getStats(file) {
    ...
    for (let fileEntry of this._fileIndex) {
      ...
      if (fileEntry.id === file.id) {
        ...
        ctx.fillStyle = colors[fileEntry.rating + 1];
        ctx.fillRect(Math.round(x), 0, Math.ceil(width), statsHeight);
      }
      x += width;
    }
    ...
    ctx.fillStyle = "#FE5";
    ctx.fillRect(Math.round(current.x), 0, Math.ceil(current.width), statsHeight);
    ...
    ctx.strokeStyle = "#888";
    ctx.beginPath();
    ctx.moveTo(x - statsHeight / 4, statsHeight);
    ctx.lineTo(x, statsHeight / 2);
    ctx.lineTo(x + statsHeight / 4, statsHeight);
    ctx.fill();
    ctx.stroke();

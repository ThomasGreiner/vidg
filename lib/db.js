...
class Database {
  ...
  getPlaylist() {
    return new Promise((resolve, reject) => {
      ...
      // TODO: add duration query parameter (e.g. "stats_duration >= 1800")
  ...
  attachTag(fileId, tagId) {
    return new Promise((resolve, reject) => {
      let stmt = this._db.prepare(`
        INSERT INTO files_tags(id_file, id_tag) VALUES ($id_file, $id_tag)
      `);
      stmt.run({
        $id_file: fileId,
        $id_tag: tagId
  ...
  detachTag(fileId, tagId) {
    return new Promise((resolve, reject) => {
      let stmt = this._db.prepare(`
        DELETE FROM files_tags
        WHERE id_file = $id_file AND id_tag = $id_tag
      `);
      stmt.run({
        $id_file: fileId,
        $id_tag: tagId

...
class Database {
  ...
  getFileIndex() {
    return new Promise((resolve, reject) => {
      this._db.all("SELECT id, rating, stats_bitrate, stats_created, stats_duration, stats_size FROM files ORDER BY rating ASC, stats_size DESC", (err, files) => {
        if (err) {
          reject(err);
        } else {
          resolve(this._normalizeFiles(files));
  ...
  getPlaylist() {
    return new Promise((resolve, reject) => {
      ...
      let stmt = this._db.prepare("SELECT path FROM files WHERE rating >= ?");
      stmt.all([rating], (err, files) => {
        if (err) {
          reject(err);
        } else {
  ...
  search(query) {
    return new Promise((resolve, reject) => {
      let stmt = this._db.prepare("SELECT id, rating, stats_created, stats_duration, stats_size FROM files WHERE path LIKE ? ORDER BY stats_size DESC");

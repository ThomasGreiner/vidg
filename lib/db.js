...
class Database {
  ...
  get comparisons() {
    return new Promise((resolve, reject) => {
      this._db.all(`
        ...
        FROM files AS a
        JOIN files AS b
        WHERE b.stats_size = a.stats_size AND b.id > a.id AND b.path > a.path
        ORDER BY a.stats_size DESC
      `, (err, comparisons) => {
        if (err) {
          reject(err);
          return;
  ...
  get fileIndex() {
    return new Promise((resolve, reject) => {
      this._db.all("SELECT id, rating, stats_bitrate, stats_created, stats_duration, stats_size FROM files ORDER BY stats_size DESC", (err, files) => {
        if (err) {
          reject(err);
        } else {
          resolve(this._normalizeFiles(files));

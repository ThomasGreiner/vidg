...
const {getStatusCharts} = require("./charts");
            ...
            if (params.filter) {
              ...
              switch (key) {
                case "duplicate": {
                  ...
                  let sameValues = new Set();
                  this._index.each((entry) => {
                    let entryValue = getEntryValue(entry);
                  ...
                  this._index.filter((entry) => sameValues.has(getEntryValue(entry)));
                  break;
                }
                case "rating":
                case "ratingmode":
                  let rating = params.filter.rating || 0;
                  let mode = params.filter.ratingmode || "equal";
                  this._index.filter((entry) => {
                    switch (mode) {
                      case "equal":

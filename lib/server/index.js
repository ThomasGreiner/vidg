...
const {getParams, getStaticFile, getStatus, getStatusData} = require("./utils");
  ...
  _getFile(fileEntry) {
    ...
    return this._db.get(fileEntry.id)
        .then((file) => {
          ...
          let status = getStatus(this._index, new Set([file.id]), totalSize);
          
          return {
            content: JSON.stringify({file, ranges, ratings, status}),
            status: 200,
